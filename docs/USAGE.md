...
**注意**: `itmux open` / `itmux add` 時に、設定した cwd でシェルが起動します（新規ウィンドウ）。全ウィンドウが既に開いている状態で `itmux open` した場合は、セッション内の全ペインへ `cd` が再送信されます。tmux-resurrect 復元後も `itmux open` で config の cwd が再適用されます。
